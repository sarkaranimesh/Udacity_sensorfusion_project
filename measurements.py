...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        pos_veh = np.ones((4,1))
        pos_veh[0:3] = x[0:3]
        pos_sens = self.veh_to_sens*pos_veh # coordinated in sensor's coordinate system
        visible = False
        if pos_sens[0] > 0:
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            if x[0] == 0:
                raise NameErro("Jacobian not defined for x[0]=0!")
            else:
                # help from https://knowledge.udacity.com/questions/828047
                hx[0,0] = self.c_i - self.f_i*pos_sens[1]/pos_sens[0] # - project from camera to image coordinates
